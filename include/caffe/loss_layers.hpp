...
template <typename Dtype>
class UnbalancedLossLayer : public LossLayer<Dtype> {
public:
    explicit UnbalancedLossLayer(const LayerParameter& param)
	: LossLayer<Dtype>(param), diff0(), diff1(), diff_() {}
	
    virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
                         const vector<Blob<Dtype>*>& top);

    virtual inline const char* type() const { return "UnbalancedLoss"; }

...
# dataDict = cPickle.load(open('imageIdFeaturePairLMDB.p', 'rb'))
# data = [value for key, value in dataDict.items()]
# X_raw = np.array(data, dtype=np.float64)
# X_2d = bh_sne(X_raw)
# X_scaled = MinMaxScaler().fit_transform(X_2d)
# cPickle.dump(X_scaled, open('2dfeatures.p','wb'))

data = cPickle.load(open('2dfeatures.p', 'rb'))
